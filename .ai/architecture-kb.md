...
| git-merge-camera2-refactor-20260509 | 2026-05-09 | 合并远程TS重构：App.js→App.tsx + 组件拆分 |
| camera2-sx-split-photo-back-hidden-fix-20260521 | 2026-05-21 | SX拍照后置被前置遮盖（非PiP分支未遵守sxBackOnTop） |
...
- **状态**: [FIXED — 视频合成 2026-04-30, 拍照合成 2026-05-21]

### SX 拍照后置被前置遮盖 — compositedImageForLayoutState 非 PiP 分支未遵守 sxBackOnTop（2026-05-21）
- **首次发现**: 2026-05-21
...
- **根因**: `compositedImageForLayoutState:front:back:highQuality:` 的非 PiP 分支（第 159–162 行）永远先画 back 再画 front（前置永远在最上层遮盖后置）。LR 布局碰巧正确（前置在右侧），但 SX 布局 `sxBackOnTop=YES` 时应该后置在上，结果前置遮盖了后置。
- **数据流验证**: JS→CameraSurface→layoutStateSnapshot→photoState，`state.sxBackOnTop` 全链路传递正确，问题在于 `compositedImageForLayoutState:` 收到了值但未使用。
- **修复**: 在非 PiP 分支中添加 `BOOL isSX = [layout isEqualToString:@"sx"];` 判断，当 `isSX && state.sxBackOnTop` 时交换合成顺序（先画 front 再画 back）。LR 布局不受影响。
- **状态**: [FIXED — 2026-05-21]
